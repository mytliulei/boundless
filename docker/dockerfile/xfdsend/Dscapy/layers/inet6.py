...
class ICMPv6MLv2Report(_ICMPv6):
    name = "MLDv2 - Multicast Listener Report"
    fields_desc = [ ByteEnumField("type", 143, icmp6types),
                    ByteField("code", 0),
                    XShortField("cksum", None),
                    XShortField("res", 0x0000),
                    XShortField("numofrec", 0x0001),
                  ]
                 
class ICMPv6MLv2Record(Packet):
    name = "MLDv2 Record"
    fields_desc = [ XByteField("rectype", 0x01),
                    XByteField("datalen", 0x00),
                    XShortField("numofsource", 0x0001),
                    IP6Field("group", "::"),
                  ]

class Srcv6List(Packet):
    name = "source ipv6 address list"
    fields_desc = [ IP6Field("sourcev6", "2001::1"),
                  ]
