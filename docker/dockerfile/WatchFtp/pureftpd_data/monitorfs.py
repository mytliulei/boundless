...
class EventHandler(pyinotify.ProcessEvent):
    ...
    def my_init(self,watch_path=None,copy_path=None,ftpbase_path=None,codelistkey='switch:codelist:key'):
        ...
        self.switch_key = 'switch:dailybuild:key'
    ...
    def processIMG(self,path,name):
        '''
        '''
        sys.stdout.write('process img file...\n')
        ...
        if not self.code_stream or not self.productname:
            print('can not get code_stream or product_name in processIMG')
            return False
    ...
    def copyIMG(self,pathname,name):
        '''
        '''
    ...
    def broadcastRedis(self,name):
        ...
        try:
            self.redis = redis.Redis(connection_pool=RedisPool)
            broadcast_str = '%s||||%s||||%s' % (self.productname,self.code_stream,os.path.join(self.ftpbase_path,name))
            cpkey = '%s %s img' % (self.code_stream,self.productname)
